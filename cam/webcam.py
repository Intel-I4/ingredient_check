...
class Webcam:    
    def __init__(self, cam_lbl, txt_lbl, ex_txt_lbl):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # GPU 사용 가능 여부에 따라 장치 선택
    ...
    def detect_and_label_objects(self, name):
        ...
        detected_objects = ['beef', 'carrot', 'chicken', 'chili', 'egg', 'garlic', 'kimchi', 'leek', 'onion', 'potato']
    ...
    def find_common_and_different(self, ingred_list, cam_list):
        ...
        if list(common_values) == []:
            ...
            different_values = (ingred_set - cam_set).union(cam_set - ingred_set)
        ...
        ex_ingred_list_str = ""
        for numm in range(len(ex_ingred_list)):
            if numm == None:
                continue
            elif numm % 5 == 0:
                ex_ingred_list_str = ex_ingred_list_str + "\n" + ex_ingred_list[numm]
            else:
                ex_ingred_list_str = ex_ingred_list_str + " " + ex_ingred_list[numm]
    ...
    def update_camera(self):
        ...
        if self.cam is not None and self.cam.isOpened():
            ...
            if ret:
                ...
                input_tensor = transform(pil_image).unsqueeze(0).to(self.device)  # 모델 입력 텐서 생성
                ...
                if detections.boxes is not None:
                    boxes = detections.boxes.data.cpu().numpy()  # 바운딩 박스 데이터를 numpy 배열로 변환
                    #print(boxes)
                    
                    # 감지된 레이블 리스트 초기화 (현재 프레임에 대해서만)
                    #current_frame_labels = []                    
                    #print(self.none_menu)
                    for box in boxes:
